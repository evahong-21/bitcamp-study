...
class CarFactory {
  ...
  // - 인스턴스를 여러개 생성할 필요가 없는 경우에 생성자를 private으로 막는다.
...
public class Exam0222 {
  ...
  public static void main(String[] args) {
    ...
    //  자동차 공장 객체를 먼저 만든다.
    //    CarFactory factory = new CarFactory(); // 컴파일 오류 == 생성자가 private이므로.

    // 생성자가 private일 경우 보통 스테틱 메서드를 통해 인스턴스를 생성한다.
    CarFactory factory = CarFactory.getInstance();

    Car2 c = factory.create("소나타");

    CarFactory factory2 = CarFactory.getInstance();

    Car2 c2 = factory2.create("티코");
    ...
    // 생성자가 private 접근으로 막혀있어
    // new 연산자를 이용하지 않고 스태틱 메서드를 호출해서 인스턴스를 생성하는 경우에 둘중하나임.
