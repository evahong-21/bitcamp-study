...
public class Exam0240 {
  public static void main(String[] args) { 
    ...
    keyboard.nextLine(); // 남아 있는 빈 문자열의 한 줄(LF 코드)을 읽어서 버린다. (OD OA)
    // nextLine은 엔터 이후에 white space까지도 문자열로 생각하므로 이부분을 방지하려면 next만 사용

    System.out.print("이름? ");
    //String name = keyboard.nextLine(); //> space까지 표현.
    String name = keyboard.next(); // space도 제외.
    keyboard.close();
